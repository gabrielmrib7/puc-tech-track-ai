  ...
  return (
    <main className="min-h-screen bg-[#faf8ff] text-[#131b2e]">
      ...
      <div className="mx-auto max-w-lg space-y-6 p-4 pb-10">
        {order ? (
          <>
            <section className="relative overflow-hidden rounded-lg bg-white p-6 shadow-sm">
              <div
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#434655]">{order.order_number}</p>
                </div>
                <span
                  className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${getBadgeStyle(
                    order.status
                  )}`}
                >
              ...
              <div className="mt-5 rounded bg-[#f2f3ff] p-4">
                <p className="text-xs font-semibold uppercase text-[#434655]">Previsão de entrega</p>
                <p className="text-xl font-semibold">
                  {order.estimated_completion
                    ? order.estimated_completion.toLocaleDateString("pt-BR")
                    : "A definir"}
                </p>
              </div>
            </section>
            ...
            <section className="rounded-lg bg-white p-6 shadow-sm">
              <h2 className="mb-6 border-b border-[#c3c6d7]/30 pb-4 text-xl font-semibold">
                Linha do Tempo
              </h2>
                ...
                <div className="space-y-5">
                  {order.history.map((item) => (
                    <div className="flex gap-3" key={item.id}>
                      ...
                      <div>
                        <p className="text-sm font-semibold">{item.description || item.action}</p>
                        <p className="text-sm text-[#434655]">{formatDate(item.created_at)}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-[#434655]">Ainda não há movimentações registradas.</p>
              )}
            </section>
          </>
        ) : (
          <section className="rounded-lg bg-white p-6 text-center shadow-sm">
            <h1 className="text-xl font-semibold">Nenhuma ordem encontrada</h1>
            <p className="mt-2 text-sm text-[#434655]">
              Ainda não há uma ordem vinculada ao seu cadastro.
            </p>
          </section>
        )}

        <Link
          className="block text-center text-sm font-semibold text-[#004ac6] hover:underline"
          href="/portal/orders"
        >
          Ver todas as ordens
        </Link>
